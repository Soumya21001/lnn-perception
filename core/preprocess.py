...
all_folders = sorted([f for f in os.listdir(main_folder) if os.path.isdir(os.path.join(main_folder, f))])
...
for split in splits:
    ...
    for video_folder in tqdm(split_map[split]):
        ...
        for frame_file in sorted(os.listdir(video_path)):
            ...
            output_frame_path = os.path.join(output_video_path, frame_file.replace('.jpg', '.pt'))
