...
from BDD100K.baseline_model import CNNBaseline, CNNLSTMBaseline
...
def main():
    ...
    model_dict = {
        ...
        "cnn_lstm_baseline": CNNLSTMBaseline(),
        "LNN_model": FlowEnhancedLNN()
    }

    for name, model in model_dict.items():
        print(f"\nEvaluating {name}...")
        model = torch.load(
            f"{name}.pth", map_location=device, weights_only=False)

        model.to(device)
        evaluate(
            model, dataloader, device, save_dir=f"eval_visuals_{name}"
        )
        print()
