...
class AVPerceptionDataset(Dataset):
    def __init__(self, frame_root, flow_root, split='train', seq_length=5, transform=None, cache_data=False):
        ...
        for vid in video_ids:
            ...
            frame_paths = sorted(
                glob(os.path.join(frame_dir, "frame_*.pt")), key=self._frame_sort_key)
            flow_paths = sorted(
                glob(os.path.join(flow_dir, "flow_*.npy")), key=self._flow_sort_key)

            if len(frame_paths) < seq_length or len(flow_paths) < seq_length - 1:
                print(
                    f"Skipping video {vid}: Not enough data ({len(frame_paths)} frames, {len(flow_paths)} flows)")
                continue
        ...
        try:
            ...
            for fp in flow_paths:
                if self.cache_data and fp in self.flow_cache:
                    ...
                    flow_np = cv2.resize(
                        flow_np, (224, 224), interpolation=cv2.INTER_LINEAR)
                    flow = torch.from_numpy(flow_np).permute(
                        2, 0, 1).float().div(224.0)
                    if self.cache_data:
